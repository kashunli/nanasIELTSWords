...
A local-first IELTS vocabulary memorization service built from one
...
The prepared input is generated by the Nana listening project (a separate
repository). Inside that project, the base cut manifest lives under:

```text
var/derived/audio_cuts/bilibili/BV1AT4y1579F
```
...
python tools\import_cut_manifest.py
...
python tools\transcribe_chapters.py --pass small
...
python tools\select_transcripts.py
...
python tools\transcribe_review.py --model medium --start-index 0 --end-index 185 --output-name pass-large.jsonl
python tools\transcribe_review.py --model medium --start-index 185 --end-index 369 --output-name pass-large-part2.jsonl
python tools\merge_review_asr.py --input pass-large.jsonl --input pass-large-part2.jsonl

# Re-select after the optional review pass and prepare deterministic batches.
python tools\select_transcripts.py
python tools\prepare_meaning_batches.py
...
python tools\generate_meanings.py

# Validate and merge the completed meaning batches.
python tools\merge_meaning_batches.py
...
python tools\audit_book_audio.py

# Generate Chinese translation audio with the selected Edge voice.
python -m pip install edge-tts
python tools\generate_translation_audio.py --voice zh-CN-YunjianNeural

# Require verified translation audio during the learner DB build.
python tools\build_content.py --require-translation-audio
